...
public class UserEditorGroupsPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            ...
            if (editable.Value)
            {
                var groupCheckBox = InformationModel.MakeObject<Panel>(group.BrowseName, Tata_Energy_Management.ObjectTypes.GroupCheckbox);
            ...
            {
                var groupLabel = InformationModel.MakeObject<Panel>(group.BrowseName, Tata_Energy_Management.ObjectTypes.GroupLabel);
